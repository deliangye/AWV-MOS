...
public:
    ...
    // Publisher
    ros::Publisher m_pub_segmented_new_scan_all;
    ros::Publisher m_pub_segmented_new_scan_static;
    ros::Publisher m_pub_segmented_new_scan_dynamic;
...
public:
    ...
    mapOptimization()
    {
        ...
        // Publish
        m_pub_segmented_new_scan_all = nh.advertise<sensor_msgs::PointCloud2>(m_cfg_s_output_pc_namespace + std::string("/segmented_new_scan_all"), 1);
        m_pub_segmented_new_scan_static = nh.advertise<sensor_msgs::PointCloud2>(m_cfg_s_output_pc_namespace + std::string("/segmented_new_scan_static"), 1);
        m_pub_segmented_new_scan_dynamic = nh.advertise<sensor_msgs::PointCloud2>(m_cfg_s_output_pc_namespace + std::string("/segmented_new_scan_dynamic"), 1);

        m_awv_mos.SaveConfigParams();
        if (m_awv_mos.m_cfg_b_write_prediction == true && !std::filesystem::exists(m_awv_mos.m_cfg_s_prediction_write_path)) 
        {
            std::filesystem::create_directories(m_awv_mos.m_cfg_s_prediction_write_path);
            std::cout << "Create prediction write path folder: " << m_awv_mos.m_cfg_s_prediction_write_path << std::endl;
        }
    }
...
/* --------------------- Graph Optimization Functions */
    ...
    void publishFrames()
    {
        ...
        {
            pcl::PointCloud<PointTypeMOS>::Ptr surrounding_keyframes_combine(new pcl::PointCloud<PointTypeMOS>());
        ...
        if (m_pub_segmented_new_scan_all.getNumSubscribers() != 0)
        {
            publishCloud(m_pub_segmented_new_scan_all, m_pres_segmented_scan, timeLaserInfoStamp, "lidar_link");
        }
        if (m_pub_segmented_new_scan_static.getNumSubscribers() != 0 || m_pub_segmented_new_scan_dynamic.getNumSubscribers() != 0)
        {
            ...
            publishCloud(m_pub_segmented_new_scan_static, segmented_new_scan_static, timeLaserInfoStamp, "lidar_link");
            publishCloud(m_pub_segmented_new_scan_dynamic, segmented_new_scan_dynamic, timeLaserInfoStamp, "lidar_link");
        }
        ...
        if (pubSLAMInfo.getNumSubscribers() != 0)
        {
            if (lastSLAMInfoPubSize != (int)cloudKeyPoses6D->size())
            {
                ...
                *cloudOut += *laserCloudSurfLastDS;
                ...
                pcl::PointCloud<PointTypeMOS>::Ptr localMapOut(new pcl::PointCloud<PointTypeMOS>());
    ...
    void run()
    {
        ...
        if(m_awv_mos.m_cfg_b_write_prediction == true)
        {
            std::ostringstream oss;
            oss << std::setw(6) << std::setfill('0') << m_i_frame_count - 1 << ".label";
            std::string prediction_file_name = oss.str();
            std::string prediction_write_file_name = m_awv_mos.m_cfg_s_prediction_write_path + prediction_file_name;
            m_awv_mos.WritePrediction(m_pres_segmented_scan, prediction_write_file_name);
        }
