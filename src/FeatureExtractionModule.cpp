...
/* Deskewing functionse */
    ...
    void cloudHandler_noloss(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        static tbb::global_control global_limit(tbb::global_control::max_allowed_parallelism, m_cfg_n_num_cpu_cores);
        ...
        deskewInfo();
        projectPointCloud_noloss();
        cloudExtraction_noloss();
        ...
        // calculateSmoothness_noloss();
        markOccludedPoints_noloss();
        extractFeatures_noloss();
        publishFeatureCloud_noloss();
        resetParameters();
    ...
    void projectPointCloud_noloss()
    {
        // range image projection
        int cloudSize = laserCloudIn->points.size();
        m_featured_scan->points.resize(cloudSize);
        m_reordered_scan->points.resize(cloudSize);
        ...
        for(int i = 0; i < cloudSize; i++)
        {
            ...
            float range = sqrt(thisPoint.x*thisPoint.x + thisPoint.y*thisPoint.y + thisPoint.z*thisPoint.z);
            float phi = atan2(thisPoint.y, thisPoint.x) * 180.0 / M_PI + 180.0;

            RangePhiIdx range_phi_idx(range, phi, thisPoint.time, i);
            m_rings_range_phi_idx_vector_m[rowIdn].push_back(range_phi_idx);
            
            PointType thisPoint2;
            pcl::copyPoint(thisPoint, thisPoint2);
            m_featured_scan->points[i] = thisPoint2;
            m_featured_scan->points[i].intensity = 0;
        }

        for(int ring = 0; ring < m_rings_range_phi_idx_vector_m.size(); ring++)
        {
            if(ring == 0)
            {
                m_rings_start_index_vector[ring] = 0;
                m_rings_end_index_vector[ring] = m_rings_range_phi_idx_vector_m[ring].size() - 1;
            }
            else
            {
                m_rings_start_index_vector[ring] = m_rings_end_index_vector[ring - 1] + 1;
                m_rings_end_index_vector[ring] = m_rings_end_index_vector[ring - 1] + m_rings_range_phi_idx_vector_m[ring].size();
            }
        }

        tbb::parallel_for
        (
            size_t(0), (size_t) m_rings_range_phi_idx_vector_m.size(), [&](size_t ring) 
            {
            std::sort(m_rings_range_phi_idx_vector_m[ring].begin(), 
                        m_rings_range_phi_idx_vector_m[ring].end(), 
                        [](const RangePhiIdx& a, const RangePhiIdx& b) 
                        {
    ...
    void cloudExtraction_noloss()
    {
        int cloudSize = m_featured_scan->points.size();
        m_points_range_vector_m.clear();
        m_points_range_vector_m.reserve(cloudSize);
        m_points_phi_vector_deg.clear();
        m_points_phi_vector_deg.reserve(cloudSize);
        m_points_idx_vector.clear();
        m_points_idx_vector.reserve(cloudSize);

        tbb::parallel_for
        (
            size_t(0), (size_t) cloudSize, [&](size_t idx) 
            {
                auto iter = std::lower_bound(m_rings_end_index_vector.begin(), m_rings_end_index_vector.end(), idx);
                int p_row = iter - m_rings_end_index_vector.begin(); 
                int p_reoredred_idx = idx - m_rings_start_index_vector[p_row];

                m_points_range_vector_m[idx] = m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx].range_m;
                m_points_phi_vector_deg[idx] = m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx].phi_deg;
                m_points_idx_vector[idx] = m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx].idx;

                if(p_reoredred_idx >= 5 && p_reoredred_idx < m_rings_range_phi_idx_vector_m[p_row].size() - 1 - 5)
                {
                    float diffRange = m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx-5].range_m + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx-4].range_m
                                    + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx-3].range_m + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx-2].range_m
                                    + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx-1].range_m - m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx].range_m * 10
                                    + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx+1].range_m + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx+2].range_m
                                    + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx+3].range_m + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx+4].range_m
                                    + m_rings_range_phi_idx_vector_m[p_row][p_reoredred_idx+5].range_m;

                    cloudCurvature[idx] = diffRange*diffRange;
                    cloudSmoothness[idx].value = cloudCurvature[idx];//diffX * diffX + diffY * diffY + diffZ * diffZ;
                    cloudNeighborPicked[idx] = 0;
                }
                else
                {
                    cloudCurvature[idx] = 0;
                    cloudSmoothness[idx].value = 0;
                    cloudNeighborPicked[idx] = 1;
                }
                cloudLabel[idx] = 0;
                cloudSmoothness[idx].ind = idx;
            }
        );
    }
...
/* Feature extraciton functions */
    ...
    void calculateSmoothness_noloss()
    {
        int cloudSize = m_featured_scan->points.size();
        for (int i = 5; i < cloudSize - 5; i++)
        {
            float diffRange = m_points_range_vector_m[i-5] + m_points_range_vector_m[i-4]
                            + m_points_range_vector_m[i-3] + m_points_range_vector_m[i-2]
                            + m_points_range_vector_m[i-1] - m_points_range_vector_m[i] * 10
                            + m_points_range_vector_m[i+1] + m_points_range_vector_m[i+2]
                            + m_points_range_vector_m[i+3] + m_points_range_vector_m[i+4]
                            + m_points_range_vector_m[i+5];            
    ...
    void markOccludedPoints_noloss()
    {
        int cloudSize = m_featured_scan->points.size();
        // mark occluded points and parallel beam points
        // for (int i = 5; i < cloudSize - 6; ++i)
        ...
        tbb::parallel_for
        (
            size_t(5), (size_t) cloudSize - 6, [&](size_t i) 
            {
                // occluded points
                float depth1 = m_points_range_vector_m[i];
                float depth2 = m_points_range_vector_m[i+1];
                float phiDiff = std::abs(m_points_phi_vector_deg[i+1] - m_points_phi_vector_deg[i]);

                if (phiDiff < 10 * ang_res_x){
                    // 10 pixel diff in range image
                    if (depth1 - depth2 > 0.3){
                        cloudNeighborPicked[i - 5] = 1;
                        cloudNeighborPicked[i - 4] = 1;
                ...
                // parallel beam
                float diff1 = std::abs(m_points_range_vector_m[i-1] - m_points_range_vector_m[i]);
                float diff2 = std::abs(m_points_range_vector_m[i+1] - m_points_range_vector_m[i]);

                if (diff1 > 0.02 * m_points_range_vector_m[i] && diff2 > 0.02 * m_points_range_vector_m[i])
                    cloudNeighborPicked[i] = 1;
            }
        );
        // }
    ...
    void extractFeatures_noloss()
    {
        // for(int idx = 0; idx < 6 * N_SCAN; idx++)
        // {
        tbb::parallel_for
        (
            size_t(0), (size_t) 6 * N_SCAN, [&](size_t idx) 
            {
                ...
                int sp = ((m_rings_start_index_vector[row] + 5) * (6 - section) + (m_rings_end_index_vector[row] - 5) * section) / 6;
                int ep = ((m_rings_start_index_vector[row] + 5) * (5 - section) + (m_rings_end_index_vector[row] - 5) * (section + 1)) / 6 - 1;
                ...
                for (int k = ep; k >= sp; k--)
                {
                    ...
                    if (cloudNeighborPicked[ind] == 0 && cloudCurvature[ind] > edgeThreshold)
                    {
                        ...
                        if (largestPickedNum <= 20){
                            cloudLabel[ind] = 1;
                            int scan_idx = m_points_idx_vector[ind];
                            m_featured_scan->points[scan_idx].intensity = 1;
                        } else {
                            break;
                        }
                        ...
                        for (int l = 1; l <= 5; l++)
                        {
                            float phiDiff = std::abs(m_points_phi_vector_deg[ind + l] - m_points_phi_vector_deg[ind + l - 1]);
                            if (phiDiff > 10 * ang_res_x)
                                break;
                            cloudNeighborPicked[ind + l] = 1;
                        }
                        for (int l = -1; l >= -5; l--)
                        {
                            int phiDiff = std::abs(m_points_phi_vector_deg[ind + l] - m_points_phi_vector_deg[ind + l + 1]);
                            if (phiDiff > 10 * ang_res_x)
                                break;
                            cloudNeighborPicked[ind + l] = 1;
                        }
    ...
    void publishFeatureCloud_noloss()
    {
        // free cloud info memory
        // freeCloudInfoMemory();
        // save newly extracted features
        cloudInfo.header = cloudHeader;
        cloudInfo.cloud_feature  = publishCloud(pubFeaturePoints,  m_featured_scan,  cloudHeader.stamp, lidarFrame);
